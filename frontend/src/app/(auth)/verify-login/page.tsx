...
function VerifyLoginContent() {
  ...
  useEffect(() => {
    ...
    const raw = sessionStorage.getItem('verify-login-fallback');
    if (!raw) return;
    try {
  ...
  useEffect(() => {
    const recoverMissingDevCode = async () => {
      ...
      try {
        ...
        if (result.devLoginCode) {
          setSuccess(`Code temporaire recupere automatiquement: ${result.devLoginCode}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        sessionStorage.removeItem('verify-login-fallback');
      }
    ...
    try {
      ...
      setSuccess(
        result.deliveredToInbox
          ...
          : `Email delivery fallback active in development. Your new login code is: ${result.devLoginCode || 'check backend logs'}.`
      );
      setResendCooldown(60);
    } catch (err) {
      setError(getErrorMessage(err));
    ...
    <div className="min-h-screen bg-card flex items-center justify-center p-8 relative overflow-hidden transition-colors">
      ...
      <div className="w-full max-w-md relative">
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">Verify login</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-4 text-lg font-medium leading-relaxed">
            Enter the 6-digit login code sent to <br />
